...
engine = create_engine("mysql+pymysql://{user}:{pw}@localhost/{db}"
                       .format(user="grant3",# user
                               pw="grant3", # passwrd
                               db="routes_db")) #database
...
def success():
    if request.method == 'POST' :
        ...
        f.save(f.filename)
        connecting_routes = pd.read_csv(r"D:\DATA SCIENTIST\DATA SCIENCE\STUDY MATERIAL\Network Analytics\connecting_routes.csv")
        ...
        routes = pd.DataFrame({"closeness":pd.Series(nx.closeness_centrality(for_g)),"Degree": pd.Series(nx.degree_centrality(for_g)),"eigenvector": pd.Series(nx.eigenvector_centrality(for_g)),"betweenness": pd.Series(nx.betweenness_centrality(for_g))}) 
       
        
        # transfering file into database by using method "to_sql"
        routes.to_sql('centralities', con = engine, if_exists = 'replace', chunksize = 1000, index= False)
        html_table = routes.to_html(classes='table table-striped')
